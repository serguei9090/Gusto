...
test.describe("Phase 6: Prep Sheet Generation", () => {
  ...
  test("should create prep sheet from recipes", async ({ page, database }) => {
    ...
    await page
      .locator('select[name="recipeId"]')
      .first()
      .selectOption({ label: "Margherita Pizza" });
    await page
      .getByLabel(/quantity|servings/i)
      .first()
      .fill("10");
    ...
    await page
      .locator('select[name="recipeId"]')
      .last()
      .selectOption({ label: "Grilled Chicken Salad" });
    await page
      .getByLabel(/quantity|servings/i)
      .last()
      .fill("5");
    ...
    const prepSheet = database.db
      ...
      .get("Monday Lunch Prep") as { id: number };
  ...
  test("should load saved prep sheet", async ({ page }) => {
    await page.goto("/");
    await page.getByRole("link", { name: "Prep Sheets" }).click();
    await waitForPageReady(page);
