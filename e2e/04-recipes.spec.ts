...
test.describe("Phase 4: Recipe Management", () => {
    ...
    test("should create a new recipe with ingredients", async ({
        ...
        await page.locator('select[name="ingredientId"]').first().selectOption({ label: /Chicken/i });
        await page.getByLabel(/Quantity/i).first().fill("150");
        ...
        const recipe = database.db
            ...
            .get("Caesar Salad") as any;

        expect(recipe).toBeTruthy();
        expect(recipe.selling_price).toBe(12.0);

        // Capture screenshot
        await capturePageScreenshot(page, "04-recipe-created");
    });

    test("should verify cost calculation accuracy", async ({ page, database }) => {
        await page.goto("/");
        await page.getByRole("link", { name: "Recipes" }).click();
        await waitForPageReady(page);
      ...
      `)
        ...
        const costText = await page.locator("[data-testid='total-cost'], .total-cost").textContent();
        const displayedCost = parseFloat(costText?.replace(/[^0-9.]/g, "") || "0");
