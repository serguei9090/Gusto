...
try {
  ...
  `);
    ...
    const applied = db.query("SELECT id FROM _migrations").all() as { id: string }[];
    ...
    runMigration("20240205_add_waste_buffer",
        "ALTER TABLE recipes ADD COLUMN waste_buffer_percentage REAL DEFAULT 0"
    );

    runMigration("20240205_add_account_number_to_suppliers",
