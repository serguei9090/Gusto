import { Button } from "@/components/ui/button";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import type { Recipe } from "@/types/ingredient.types";

interface RecipeDeleteDialogProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  recipe: Recipe | null;
  isUsageWarning: boolean;
  onConfirm: () => void;
  isLoading: boolean;
}

export const RecipeDeleteDialog = ({
  isOpen,
  onOpenChange,
  recipe,
  isUsageWarning,
  onConfirm,
  isLoading,
}: RecipeDeleteDialogProps) => {
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="text-destructive flex items-center gap-2">
            Confirm Deletion
          </DialogTitle>
          <DialogDescription className="pt-2 text-foreground">
            {isUsageWarning ? (
              <div className="bg-destructive/10 p-3 rounded-md text-destructive font-medium border border-destructive/20 mb-2">
                <p className="font-bold mb-1">Warning: Used as Sub-Recipe</p>
                <p className="text-sm">
                  This recipe is used inside other recipes. Deleting it will
                  cause calculation errors in those recipes.
                </p>
              </div>
            ) : (
              "Are you sure you want to delete this recipe? This action cannot be undone."
            )}
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-2 mt-4">
          <p className="text-sm font-semibold px-1">
            Deleting: <span className="text-primary">{recipe?.name}</span>
          </p>
        </div>
        <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 mt-4">
          <button
            type="button"
            className="inline-flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 text-sm font-medium ring-offset-background transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 h-12 sm:h-9"
            onClick={() => onOpenChange(false)}
            disabled={isLoading}
          >
            Cancel
          </button>
          <Button
            variant="destructive"
            onClick={onConfirm}
            disabled={isLoading}
            className="h-12 sm:h-9 font-bold"
          >
            {isLoading ? "Deleting..." : "Delete Permanently"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
  );
};
