                    ...
                    {isLoading ? (
                        ...
                        <div className="space-y-8" id="printable-recipe">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <Badge variant="outline" className="mb-2 print:text-black print:border-black">
                                        {selectedRecipe.category || "Uncategorized"}
                                    </Badge>
                                    <h1 className="text-3xl font-bold tracking-tight text-foreground">{selectedRecipe.name}</h1>
                                </div>

                                <div className="flex gap-6 text-muted-foreground print:text-black">
                                    <div className="flex items-center gap-2">
                                        <Users className="h-4 w-4" />
                                        <span>Yield: <span className="font-medium text-foreground print:text-black">{selectedRecipe.servings} servings</span></span>
                                    </div>
                                    {selectedRecipe.prepTimeMinutes && (
                                        <div className="flex items-center gap-2">
                                            <Clock className="h-4 w-4" />
                                            <span>Prep Time: <span className="font-medium text-foreground print:text-black">{selectedRecipe.prepTimeMinutes} mins</span></span>
                                        </div>
                                    )}
                                </div>

                                {selectedRecipe.description && (
                                    <p className="text-muted-foreground print:text-black">{selectedRecipe.description}</p>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 print:grid-cols-3">
                                <Card>
                                    <CardHeader className="pb-2">
                                        <CardTitle className="text-sm font-medium text-muted-foreground">Cost per Serving</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                ...
                                <Card>
                                    ...
                                    <CardContent>
                                        <div className="text-2xl font-bold">${selectedRecipe.sellingPrice?.toFixed(2) || "0.00"}</div>
                                    </CardContent>
                                </Card>
                                <Card className={
                                    (selectedRecipe.profitMargin || 0) < 20 ? "border-destructive/50 bg-destructive/10" :
                                        (selectedRecipe.profitMargin || 0) < 30 ? "border-yellow-500/50 bg-yellow-500/10" :
                                            "border-green-500/50 bg-green-500/10"
                                }>
                                    <CardHeader className="pb-2">
                                        <CardTitle className="text-sm font-medium text-foreground">Profit Margin</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        <div className={`text-2xl font-bold ${(selectedRecipe.profitMargin || 0) < 20 ? "text-destructive" :
                                                (selectedRecipe.profitMargin || 0) < 30 ? "text-yellow-700 dark:text-yellow-500" :
                            ...
                            {/* Ingredients Table */}
                            <div className="space-y-4">
                                <h3 className="text-lg font-semibold border-b pb-2">Ingredients & Cost Breakdown</h3>
                                            ...
                                            {selectedRecipe.ingredients.map((ing) => (
                                                <TableRow key={ing.id} className="print:border-b-gray-200">
                                                    ...
                                                    <TableCell className="text-muted-foreground">
                                                        ${ing.currentPricePerUnit?.toFixed(2)} / {ing.ingredientUnit}
                                                    </TableCell>
                            ...
                            {selectedRecipe.cookingInstructions && (
                                <div className="space-y-4 break-before-page">
                                    <h3 className="text-lg font-semibold border-b pb-2">Cooking Instructions</h3>
