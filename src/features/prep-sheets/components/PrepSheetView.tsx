...
export function PrepSheetView({ sheet, onClose, onSave, showSaveButton }: PrepSheetViewProps) {
                ...
                <CardHeader className="flex flex-row items-center justify-between border-b print:border-none">
                    <div className="space-y-1.5">
                        ...
                        <CardDescription>
                            ...
                            {sheet.shift && ` • ${sheet.shift.charAt(0).toUpperCase() + sheet.shift.slice(1)} Shift`}
                ...
                <CardContent className="space-y-6 pt-6 print:p-0">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 print:grid-cols-2">
                            {sheet.recipes.map((recipe, idx) => (
                                <div key={idx} className="flex justify-between items-center bg-muted/30 p-3 rounded-md border print:border-gray-200">
                                    <span className="font-medium">{recipe.recipeName}</span>
                                    <Badge variant="outline">{recipe.requestedServings} servings</Badge>
                                </div>
                            ))}
                        </div>
                    </div>
                                    ...
                                    <TableRow key={idx}>
                                        ...
                                        <TableCell className="text-sm text-muted-foreground">
                                            <ul className="list-disc list-inside">
                                                {item.recipeBreakdown.map((breakdown, bIdx) => (
                                                    <li key={bIdx}>
                                                        {breakdown.recipeName}: {breakdown.qty.toFixed(2)} {item.unit}
                                                    </li>
                                                ))}
                                            </ul>
                                        </TableCell>
                    ...
                    {sheet.notes && (
                        <div className="bg-yellow-50 p-4 rounded-md border border-yellow-100 text-yellow-800 print:bg-transparent print:border-gray-200 print:text-black">
                            <h4 className="font-semibold text-sm uppercase mb-1 opacity-75">Notes</h4>
                            <p>{sheet.notes}</p>
                        </div>
                    )}
                </CardContent>
