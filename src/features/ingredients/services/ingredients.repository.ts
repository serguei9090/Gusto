...
export class IngredientsRepository {
    async create(data: CreateIngredientInput): Promise<Ingredient> {
        const result = await db.insertInto("ingredients")
            .values({
                name: data.name,
    ...
    async getById(id: number): Promise<Ingredient | null> {
        const row = await db.selectFrom("ingredients")
            .selectAll()
            .where("id", "=", id)
            .executeTakeFirst();
    ...
    async getAll(): Promise<Ingredient[]> {
        const rows = await db.selectFrom("ingredients")
            .selectAll()
            .orderBy("name", "asc")
            .execute();
    ...
    async update(id: number, data: UpdateIngredientInput): Promise<Ingredient | null> {
        ...
        const result = await db.updateTable("ingredients")
            .set(updateData)
            .where("id", "=", id)
            .executeTakeFirst();
    ...
    async delete(id: number): Promise<boolean> {
        const result = await db.deleteFrom("ingredients")
            .where("id", "=", id)
            .executeTakeFirst();
