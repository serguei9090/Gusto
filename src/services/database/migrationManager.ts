...
export const migrations: Migration[] = [
    {
        ...
        up: async (db) => {
            await db.execute("ALTER TABLE recipes ADD COLUMN waste_buffer_percentage REAL DEFAULT 0");
        },
    },
    {
    ...
    },
    {
        ...
        up: async (db) => {
            try {
                await db.execute("ALTER TABLE ingredients ADD COLUMN currency TEXT DEFAULT 'USD'");
            } catch (e) { }
            try {
                await db.execute("ALTER TABLE recipes ADD COLUMN currency TEXT DEFAULT 'USD'");
            } catch (e) { }
        },
    },
];
...
export async function runMigrations(db: Database) {
  ...
  `);
    ...
    const applied = await db.select<Array<{ id: string }>>("SELECT id FROM _migrations");
        ...
        if (!appliedIds.has(migration.id)) {
            ...
            try {
                ...
                if (errorMessage.includes("duplicate column name")) {
                    console.log(`ℹ️ Column already exists, marking ${migration.id} as applied.`);
                    await db.execute("INSERT INTO _migrations (id) VALUES ($1)", [migration.id]);
                } else {
