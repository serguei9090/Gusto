...
import type { ConfigItem } from "@/modules/core/settings/services/config.repository";
...
export const ConfigItemManager = ({
  ...
  const filteredItems = items.filter((item: ConfigItem) => item.type === type);
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {filteredItems.map((item: ConfigItem) => (
          <Reorder.Item
            key={item.id}
            value={item}
