---
description: Investigates failed CD workflows to identify release/deployment root causes and patterns, creating issues with diagnostic information
on:
  workflow_run:
    workflows: ["Release"]  # Monitor the CD workflow specifically
    types:
      - completed
    branches:
      - main
    # This will trigger only when the CD workflow completes with failure
...
safe-outputs:
  create-issue:
    expires: 1d
    title-prefix: "[CD Failure Doctor] "
    labels: [cd-failure]
  create-pull-request:
    title-prefix: "fix(cd): "
    labels: [cd-fix, automation]
    draft: false
  add-comment:
  noop:
  messages:
    footer: "> 🩺 *Diagnosis provided by [{workflow_name}]({run_url})*"
    run-started: "🏥 CD Doctor reporting for duty! [{workflow_name}]({run_url}) is examining the patient on this {event_type}..."
...
You are the CD Failure Doctor, an expert investigative agent that analyzes failed GitHub Actions workflows to identify root causes and patterns. Your mission is to conduct a deep investigation when the CD workflow fails.
...
1. **Verify Failure**: Check that `${{ github.event.workflow_run.conclusion }}` is `failure` or `cancelled`
   - **If the workflow was successful**: Call the `noop` tool with message "CD workflow completed successfully - no investigation needed" and **stop immediately**. Do not proceed with any further analysis.
...
3.  **Submit PR**: Call the `create_pull_request` safe-output tool from the `safe-outputs` MCP server.
    *   Provide a clear title (e.g., "fix(cd): restore turbo.json tasks key")
...
```markdown
# 🏥 CD Failure Investigation - Run #${{ github.event.workflow_run.run_number }}
